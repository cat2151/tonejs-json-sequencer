// Tone.js JSON Sequencer - Playback Viewer (Piano-Roll) for Streaming Demo
import type { SequenceEvent } from '../demo-types.js';
// @ts-ignore - Using built library
import { parseNDJSON, type EventScheduledInfo } from '../../../dist/index.mjs';
...
export class PlaybackViewer {
  ...
  private trackLines: Map<number, HTMLDivElement> = new Map();
  private trackBodies: Map<number, HTMLDivElement> = new Map();
  private durationSeconds = 0;
  private loopEnabled = false;
  private loopWaitSeconds = 0;
  ...
  startPositionUpdates(loopEnabled: boolean, loopWaitSeconds: number): void {
  ...
  flashEvent(info: EventScheduledInfo): void {
  ...
  private updateProgressFromNow(): void {
  ...
  private updatePositionLine(positionSeconds: number): void {
    if (this.durationSeconds <= 0) {
  ...
  private buildTracks(events: SequenceEvent[]): { tracks: PlaybackTrackData[]; totalDuration: number } {
