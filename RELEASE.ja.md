...
3. **gitタグを作成してプッシュ**
   ...
   **重要:** タグのバージョンはpackage.jsonのバージョンと一致する必要があります。ワークフローはこれを検証し、一致しない場合は失敗します。

4. **GitHub Actionsが自動的に以下を実行:**
   - 依存関係のインストール
   - タグバージョンがpackage.jsonバージョンと一致することを検証
   - 配布ファイルのビルド
...
- タグバージョンがpackage.jsonバージョンと一致することを検証（タグトリガーの場合）
...
- タグのバージョンはpackage.jsonのバージョンと一致する**必要があります**（一致しない場合、ワークフローは失敗します）
