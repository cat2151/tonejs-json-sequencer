...
# Why
- ※Temporarily generated by ChatGPT. This section may be rewritten in the future.
- ※In fact, much of this is "planned for future implementation."
...
- **Live Editing**: Edit the sequence during playback — changes are reflected in real-time without restarting.
...
- `LFO` - Create and start a low-frequency oscillator to modulate a parameter path
...
- Articulation
  - Done: Delay Vibrato
  - Pitch envelope where attack is -200 cents, then changes to 0 cents ※Not attack-linked, but independent like delay vibrato.
...
- Consideration of a tone library
  - ※Eventually, this might be spun off as a separate project, similar to `tonejs-mml-to-json`.
  - Past issues:
...
- Why-related: Distinction from DAWs
  ...
  - `tonejs-json-sequencer`
    - ※Or browsers, WebAudio, Tone.js, and non-DAW-like music applications in general across multiple platforms.
    - For experimentation:
      - New UIs
...
# Out of Scope
- Node ID assignment → This will be handled by a higher layer, for example, `tonejs-mml-to-json` or a JSON post-processor at a lower layer.
- Generating delay vibrato on and off for all notes → Same as node ID assignment.
- Other JSON data processing → Same as node ID assignment.
